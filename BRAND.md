...
- **2026-07-05** — R4 amendment (user decision): Essentials craft boundary relaxed from "no enamel" to **entry-tier + silent voice**. First 3 Essentials SKUs shipped: 珐琅球系列 0075/0076/0077 (enamel bead + Swarovski pearl, $49–89 via `priceGlobal`, series `enamel-bead`). Enamel beads are simple fired beads, not cloisonné/plique bench work — the wirework moat stays Atelier-exclusive. Silent-PDP rule enforced (no 工坊 / no Hu Yan on these PDPs); never home hero / never curated 6 unchanged. 3 more bead SKUs (莲花手串 / 锦鲤耳坠 / 锦鲤项链) pending photos in WeChat inbox flow.
